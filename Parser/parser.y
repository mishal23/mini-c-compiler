...
VDECI: VDECID | VDECID ':' SEXP;
VDECID : ID | ID '[' NUM_CONST ']';
TSPEC : RTSPEC | ;
RTSPEC : INT|CHAR|FLOAT|DOUBLE|LONG|SHORT;
FDEC : TSPEC ID '(' PARAMS ')' STMT | ID '(' PARAMS ')' STMT;
PARAMS : PARAML | ;
PARAML : PARAML ';' PARAMTL | PARAMTL;
PARAMTL : TSPEC PARAMIDL;
PARAMIDL : PARAMIDL ',' PARAMID | PARAMID ;
PARAMID : ID | ID '[' ']';
...
SUMOP : PLUS | MINUS ;
TERM : TERM MULOP UREXP | UREXP ;
MULOP : MUL | DIV | MOD ;
UREXP : UOP UREXP | FACTOR ;
UOP : MINUS | MUL ;
FACTOR : IMUT | MUT ;
